...
class Example4:
    ...
    # showInfo needs to know the master self.master = master
    # Create a frame to hold the widgets
    frame = Frame(master)
    # Create a Label to display a header self.headLbl = Label(frame, text="Widget Demo Program", relief=RIDGE) self.headLbl.pack(side=TOP, fill=X)
...
self.colorLabel = Label(rightColumn, text="Select a color") self.colorLabel.pack(expand=YES, fill=X)
entryText = StringVar(master)
entryText.set("Select a color")
self.colorEntry = Entry(rightColumn, textvariable=entryText) self.colorEntry.pack(expand=YES, fill=X)
...
if (len(btnList) > 0):
self.radioVal.set(btnList[0]) self.updateColor()
for btn in self.radioBtns: btn.pack(anchor=W)
 # Make the frames visible
leftColumn.pack(side=LEFT, expand=YES, fill=Y) rightColumn.pack(side=LEFT, expand=YES, fill=BOTH) centerFrame.pack(side=TOP, expand=YES, fill=BOTH)
...
Button(spacerFrame, text="Info", command=self.showInfo).pack(side=TOP, fill=X) # Create the Quit Button
Button(spacerFrame, text="Quit!", command=self.quit).pack(side=TOP, fill=X) spacerFrame.pack(side=TOP, expand=YES, fill=BOTH)
